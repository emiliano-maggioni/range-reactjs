...
import styles from "./RangeFixedValues.module.css"
import { coordsRange, rangeProps } from 'utility/types';

const RangeFixedValues = ({minValue, maxValue, minValueInput, maxValueInput, updateMinValue, updateMaxValue, stepRange}: rangeProps) => {
    ...
    const dragEnd = (event: React.DragEvent<HTMLDivElement>,markerId:number) => {
        ...
        if(markerId == 1){
            ...
            let fixedVal = getRangeValue(newPos);
            updateMinValue(fixedVal);
        }  
        else{
            if(newPos <= marker1Pos)
                newPos = marker1Pos+markerSize; 
            let fixedVal = getRangeValue(newPos);
            updateMaxValue(fixedVal);
        }  
    ...
    const getRangeValue = (pos:number) => {
        ...
        let closest = stepRange!.reduce(function(prev, curr) {
...
export default RangeFixedValues;